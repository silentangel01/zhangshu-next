<!-- archived: 2026-05-25; task: review-page-ui-polish -->
...
- `frontend/src/pages/review/ReviewCheckPage.vue`
  ...
  - 用户截图反馈显示：页面虽然已经形成两列和下方结果区，但整体仍不协调，主要问题是信息密度过低、空状态区域过大、按钮层级过重、词库表单和检查表单都像“大块容器”，缺少明确的主次关系。
  - 截图中“项目列表”按钮位于页面右上角但与主标题区关系较弱；“检查功能只提示，不会自动修改正文”与检查范围之间留白偏大；“开始检查”和“添加词条”按钮都采用大面积满宽样式，导致两个面板视觉重量过重。
  - 词库面板中“导入词库 / 导出词库”属于低频操作，当前放在一级按钮区会分散注意力；空词条区域和检查结果空状态高度偏大，页面显得松散。
...
5. 检查功能页面布局优化
   ...
   - 根据用户截图，进一步做视觉层级重排：
     - 页面顶部建立清晰 header：左侧保留“返回写作页 / 检查 / 违禁词 / 敏感词检查”，右侧“项目列表”作为 header action 对齐到标题区域，不要孤立漂浮。
     - 压缩检查面板中说明文本与“检查范围”之间的垂直空白，让说明文字更接近标题或改为紧凑提示行。
     - “检查范围”三个选项不要做成过大的卡片按钮；应改为更轻量的 segmented control，高度更低、间距更紧凑、文本居中。
     - “当前章节”选择器和“开始检查”按钮不必都满宽占据整张卡片。可保留选择器满宽，但将“开始检查”改为右对齐或中等宽度主按钮；若仍满宽，需降低高度和上下留白。
     - 检查面板应形成从“范围选择 -> 章节选择 -> 开始检查”的紧凑流程，不要出现大段空白断层。
   - 词库面板根据截图做密度优化：
     - 将“导入词库 / 导出词库”收纳到“更多”二级菜单，一级区域只保留“添加词条”或主要编辑动作。
     - 词库新增表单不要全部纵向铺满。建议使用紧凑 grid：
       - 第一行：匹配词、严重程度。
       - 第二行：建议或说明。
       - 操作按钮使用中等宽度并右对齐，避免“添加词条”整行满宽造成视觉压迫。
     - 空词条区域高度应降低，作为轻量 empty state 即可，不要形成大块虚线框。
   - 调整“检查范围”：
   ...
   - 调整页面三个核心容器的竖直轴对称：
     ...
     - 不要让某个面板单独贴边或偏离页面中心线。
     - 检查结果区域在无结果时不应过高。空状态高度建议控制在能表达状态即可，避免下方形成一整块空洞。
     - 三个核心容器的间距应形成稳定节奏：顶部两列之间的 gap、两列与结果区之间的 gap、结果区内部 padding 保持同一组 spacing token。
...
- 检查页面整体不再呈现“大块空白 + 大面积满宽按钮”的松散状态；标题区、检查流程、词库编辑区、空状态之间有清晰主次。
- “项目列表”按钮与页面 header 对齐，视觉上属于标题区域操作。
- “开始检查”和“添加词条”按钮层级合理，不再同时以大面积满宽按钮抢占视觉重心。
- 词库导入、导出等低频操作如被收纳到二级菜单，入口必须清晰可见。
- 无词条和无检查结果的 empty state 面积合理，不应造成页面下半部大面积空洞。
